# Teljes folyamat: Chat egy hívással JWT-val

Ez a dokumentum bemutatja, hogyan lehet egy teljes chat folyamatot végrehajtani JWT token hitelesítéssel. A folyamat két lépésből áll:
...
## Lépés 2: Chat hívás JWT tokennel

A lekért JWT token használatával chat üzenet küldhető a **POST /chat/stream** endpointon keresztül (SSE stream válasz).
...
curl -X POST "$BASE_URL/chat/stream" \
  ...
  -d '{"channel": "internal", "text": "Mi a cégetek fő szolgáltatása?", "session_id": "chat-session-123"}'
```
...
Ez a példa mutatja be a teljes folyamatot: bejelentkezés → chat hívás.
...
// 2. Chat hívás
const chat_url = `${BASE_URL}/chat/stream`;
const chat_payload = {
    channel: "internal",
    text: "Mi a cégetek fő szolgáltatása?",
    session_id: "chat-session-123"
...
- **Token élettartam**: A JWT token élettartama általában 1 óra (3600 másodperc). Ha a token lejárt, új bejelentkezést kell végezni.
