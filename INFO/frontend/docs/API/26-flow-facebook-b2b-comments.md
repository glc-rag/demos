...
- **B2B integrációhoz** hozz létre (vagy használj) API kulcsot **`fb_comment` scope-pal**: ez a jogosultság a komment **lista** és **küldés** mellett a Facebook modulban a **RAG dokumentumkeresés** (`…/comments/retrieve`), a **shop termékkeresés** (`…/comments/retrieve-products`) és a **format-reply** végpontokra is elég — **nem** kell ehhez külön `rag` vagy `shopping` scope a Facebook válaszoló hívásaihoz. Részletek: [03-auth-api-key.md](./03-auth-api-key.md).
...
## Válaszszöveg (`our_reply`) — automata worker és B2B (retrieve / format-reply)

### Automatikus mód (háttér worker)

A háttér **`fb_comment_worker`** a webhook / sor alapján dolgozza fel a kommentet:
...
3. A **`fb_commenter`** LLM végponttal generál választ úgy, hogy **csak ezekből a szövegekből** dolgozzon.
4. Ha nincs megfelelő csoport / üres lista: a profil **fallback** beállításai és LLM léphet életbe.

Az automata válasz tehát **adatcsoport + tartalomlista + dedikált FB LLM**; külön HTTP hívással nem indítható (webhook → sor → worker).

### Jóváhagyásos felület és B2B — **találatlista → kiválasztás → formázás**
...
Minden fenti útvonal: **JWT** vagy **`X-API-Key`** + scope **`fb_comment`** — tehát a felületen lévő **„Dokumentumok keresése” (RAG)** és **„Keresés a shopban”** ugyanilyen hitelesítéssel hívható **külső rendszerből is** (B2B), mint a komment lista vagy a `send`.

**Megjegyzés:** ez a **tenant dokumentum / shop index** alapú út; **nem** ugyanaz az adatforrás, mint a worker **adatcsoport (`FbContent`)** listája.
...
Ha a **`reply_mode`** értékét API-n szeretnéd **beállítani vagy lekérni** (`GET` / `PUT /admin/fb/profiles/{profile_id}`), szükség van **tenant admin** JWT-re. A bejelentkezés: `POST /auth/login`. Részletesen: [01-auth-jwt.md](./01-auth-jwt.md).
